...
var util = {
  /*
   * Verifies if a value is a none empty string
   * @param {string} x
   * @return {boolean}
   * */
  ...
  /*
   * Verifies is a value represents an integer
   * @param {string} x
   * @return {boolean}
   * */
  isIntegerOrIntegerString: function(x) {
    return typeof x === "number" && x.toString().search(/^-?[0-9]+$/) === 0 || typeof x === "string" && x.search(/^-?[0-9]+$/) === 0;
  },
  ...
  fillSelectWithOptions: function (m, selList, keyProp, displayProp) {
    var optionEl=null, keys, obj=null, i;
    keys = Object.keys( m);
